...
import { rideMetrics } from "./cycling";
...
export interface ActivityMetrics {
  ...
  /** Normalized power, watts. Real power meters and full resolution only. */
  npW: number | null;
 ...
 * recording rate is not silently mis-averaged. Null when the trace is shorter
 * than one window, which is why this can only be computed at full resolution:
 * the 400-point downsample of an hour-long ride steps ~9 seconds at a time and
 * would average away exactly the surges the metric exists to capture.
 */
export function normalizedPower(
  ...
  if (t.length < 2 || t[t.length - 1] - t[0] < NP_WINDOW_S) return null;

  // Sample i's power holds over [t[i], t[i+1]). `acc` is the integral of power
  // over the segments ending at or before the cursor, from t[lo] onwards; `lo`
  ...
  for (let i = 1; i < t.length; i++) {
    acc += w[i - 1] * (t[i] - t[i - 1]);
    const windowStart = t[i] - NP_WINDOW_S;
    while (t[lo + 1] <= windowStart) {
      acc -= w[lo] * (t[lo + 1] - t[lo]);
      lo++;
    }
    if (windowStart < t[0]) continue;
    // Segment lo straddles the window start; drop only the part before it.
    const partial = w[lo] * (windowStart - t[lo]);
...
export function computeStreamMetrics(
  ...
  return {
    ...
    decouplingPct: basis
      ...
      : null,
    npW:
      activity.hasRealPower && streams.watts ? normalizedPower(streams.timeS, streams.watts) : null,
    hrZoneSecs:
