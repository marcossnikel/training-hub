...
function renderWeeks() {
  return renderTwoWeeks([
    activity(THIS_WEEK, { tss: 60.8, moving_time_s: 3332, distance_km: 10.03 }),
    activity(at(2026, 7, 22), { tss: 48.2, moving_time_s: 2711, distance_km: 20.6 }),
    activity(LAST_WEEK, { tss: 59.1, moving_time_s: 3069, distance_km: 9.51 }),
  ]);
}
...
describe("TotalsTable", () => {
  ...
  it("marks a drop with the muted color, not a warning color", () => {
    renderTwoWeeks([
      activity(THIS_WEEK, { tss: 40, distance_km: 5 }),
      activity(LAST_WEEK, { tss: 100, distance_km: 15 }),
    ]);
  ...
  it("prints a zero period as zeros, so a rest week does not read as missing data", () => {
    ...
    expect(valuesOf(container, 0)).toEqual(["This week", "0", "0h 00m", "0.0 km", "0 m", "0"]);
    expect(valuesOf(container, 0).join(" ")).not.toContain("–");
  });

  it("prints a delta wherever the rounded values differ", () => {
    // 100.6 over 100.4 shows 101 over 100: a visible step, so it carries a delta.
    const { container } = renderTwoWeeks([
      activity(THIS_WEEK, { tss: 100.6 }),
      activity(LAST_WEEK, { tss: 100.4 }),
    ]);
    expect(valuesOf(container, 0)[1]).toBe("101");
    expect(valuesOf(container, 1)[1]).toBe("100");
    expect(deltasOf(container, 0)).toEqual(["+1"]);
  });

  it("stays quiet where the rounded values are the same, even when the raw ones differ", () => {
    // 100.4 over 99.6 both print 100, so a "+1" there would point at nothing.
    const { container } = renderTwoWeeks([
      activity(THIS_WEEK, { tss: 100.4 }),
      activity(LAST_WEEK, { tss: 99.6 }),
    ]);
    expect(valuesOf(container, 0)[1]).toBe("100");
    expect(valuesOf(container, 1)[1]).toBe("100");
    expect(deltasOf(container, 0)).toEqual([]);
  });
  ...
  it("reports an hours change at the minute precision the hours values print in", () => {
    ...
    expect(valuesOf(container, 0)[2]).toBe("7h 12m");
    expect(valuesOf(container, 1)[2]).toBe("7h 11m");
