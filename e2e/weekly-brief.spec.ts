...
test("weekly brief renders the completed-week route with source evidence", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: "This week, in context." })
  ).toBeVisible();
  await expect(page.getByRole("link", { name: "Weekly brief" })).toHaveAttribute(
    "aria-current",
    "page"
  );
  await expect(page.getByText("Method, not a verdict")).toBeVisible();
  await page.getByText(/^Review the \d+ source activities$/).click();
